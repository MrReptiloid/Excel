...
namespace Excel
{
    ...
    public class Calculation
    {
        ...
        public double Evaluate(string input, List<ICell> Cells, int cols)
        {
            if (input[0] == '=')
                ...
                catch (Exception ex)
                {
                    ...
                    else
                    {
                        ...
                        if (input[i] >= 'A' && input[i] <= 'Z')
                        {
                            ...
                            int row = int.Parse(operand);
                            string newInput = Cells[cols*(row-1) + col].value;
                            double res = Evaluate(newInput, Cells, cols);
                            _operands.Push(res);
                        }
        ...
        {
            if (c.ToString().IndexOfAny(new[] { '+', '-', '*', '/' }) != -1)
                return true;
            else return false;
        }
